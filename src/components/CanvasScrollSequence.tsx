  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 h-[100dvh] overflow-hidden bg-background">
        ...
        {/* Animated glow */}
        <motion.div
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60vw] h-[60vw] max-w-[800px] max-h-[800px] rounded-full pointer-events-none"
          style={{
          ...
          <>
            <TextOverlay scrollStart={0} scrollEnd={0.18} direction="up">
              <div className="text-center px-4 sm:px-6">
                <motion.p
                  className="text-label mb-2 sm:mb-4 text-[10px] sm:text-xs"
                ...
                <motion.h1
                  className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-9xl font-display font-light tracking-tighter mb-3 sm:mb-6"
                ...
                <motion.p
                  className="text-base sm:text-xl md:text-2xl text-muted-foreground font-light max-w-xl mx-auto"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
            ...
            <TextOverlay scrollStart={0.2} scrollEnd={0.38} direction="left">
              <div className="text-center px-4 sm:px-6">
                <p className="text-label mb-2 sm:mb-4 text-[10px] sm:text-xs">Engineered Performance</p>
                <h2 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-display font-light tracking-tight mb-2 sm:mb-4">
                  Zero Gravity
                </h2>
                <p className="text-sm sm:text-lg text-muted-foreground max-w-md mx-auto">
                  Revolutionary cushioning that makes every step feel weightless
                </p>
              </div>
            </TextOverlay>
            ...
            <TextOverlay scrollStart={0.4} scrollEnd={0.58} direction="right">
              <div className="text-center px-4 sm:px-6">
                <p className="text-label mb-2 sm:mb-4 text-[10px] sm:text-xs">Premium Materials</p>
                <h2 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-display font-light tracking-tight mb-2 sm:mb-4">
                  Crafted Excellence
                </h2>
                <p className="text-sm sm:text-lg text-muted-foreground max-w-md mx-auto">
            ...
            <TextOverlay scrollStart={0.6} scrollEnd={0.78} direction="up">
              <div className="text-center px-4 sm:px-6">
                <p className="text-label mb-2 sm:mb-4 text-[10px] sm:text-xs">Design Philosophy</p>
                <h2 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-display font-light tracking-tight mb-2 sm:mb-4">
                  Form Follows Motion
                </h2>
                <p className="text-sm sm:text-lg text-muted-foreground max-w-md mx-auto">
            ...
            <TextOverlay scrollStart={0.8} scrollEnd={1} direction="up">
              <div className="text-center px-4 sm:px-6">
                <p className="text-label mb-3 sm:mb-6 text-[10px] sm:text-xs">The Future is Now</p>
                <h2 className="text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-display font-light tracking-tight mb-4 sm:mb-8">
                  Step Into Tomorrow
                </h2>
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center pointer-events-auto">
                  <a href="#services" className="btn-luxury text-xs sm:text-sm px-6 py-3 sm:px-8 sm:py-4">
                    Explore Collection
                  </a>
                  <a href="#about" className="btn-luxury-outline text-xs sm:text-sm px-6 py-3 sm:px-8 sm:py-4">
                    Our Story
                  </a>
                </div>
              </div>
        ...
        {!isLoading && (
          ...
          >
            <p className="text-[10px] sm:text-xs text-muted-foreground tracking-widest uppercase mb-2 sm:mb-3">
              Scroll to explore
            </p>
            <motion.div
              className="w-5 h-8 sm:w-6 sm:h-10 border border-muted-foreground/50 rounded-full flex items-start justify-center p-1"
        ...
        {!isLoading && (
          <motion.div
            className="fixed right-4 sm:right-6 top-1/2 -translate-y-1/2 w-0.5 sm:w-1 h-24 sm:h-32 bg-border/30 rounded-full overflow-hidden hidden sm:block"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 2 }}
          >
